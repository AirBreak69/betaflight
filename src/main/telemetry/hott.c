...
void hottPrepareGPSResponse(HOTT_GPS_MSG_t *hottGPSMessage)
{
    ...
    int32_t altitude_m = gpsSol.llh.alt_cm / 100;
    if (!STATE(GPS_FIX)) {
        altitude_m = getEstimatedAltitude_cm() / 100;
    }

    const uint16_t hottGpsAltitude = (uint16_t)constrain(altitude_m + HOTT_GPS_ALTITUDE_OFFSET, 0 , UINT16_MAX); // gpsSol.llh.alt in m ; offset = 500 -> O m
...
{
    const uint16_t hottEamAltitude = (getEstimatedAltitude_cm() / 100) + HOTT_EAM_OFFSET_HEIGHT;
